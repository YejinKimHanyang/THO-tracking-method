...
PIXEL_DISTANCE_THRESHOLD_2 = 50
FEATURE_MATCHING_THRESHOLD = 20
...
def inference_image(model, image):   
    ...
    for i in range(len(label_list)):
        if label_list[i] == 7: #truck
            ...
            continue   
        elif label_list[i] == 12: #parking meter
            continue
        elif label_list[i] == 26: #handbag
            continue
        elif label_list[i] == 27: #tie
            continue
...
def main():
    ...
    writer = cv2.VideoWriter('./output6_changed_yejin.avi', cv2.VideoWriter_fourcc(*'DIVX'), 30, (1040, 600))
                        ...
                        for t in range(len(tracking_objects[label])):
                            ...
                            ## with feature distance
                                ...
                                if pixel_distance < PIXEL_DISTANCE_THRESHOLD * 2: # 2x small
                                    ...
                                    #제일 많은 len(matches) 를 가지고 있는 tracked object의 TO를 t로 지정하기
                                    if len(matches) > FEATURE_MATCHING_THRESHOLD:
                                        if len(matches) > best_match_count:
                                            ...
                                            print(past_pixel_distance)

                                            #added by yejin
                                            #feature matching threshold of the previous best match object after checking that the current pixel_distance is less than
                                            #previous object of the best match's tracked object
                                            t_obj2 = tracking_objects[label][best_match].find_last_object()
                                            matches2 = matcher.match(obj.features, t_obj2.features)
                                            #현재 t 를 기준으로 비교하기
                                            if pixel_distance < past_pixel_distance:
                                                #현재 t 의 matches count가 best match t의 ㅣ전 t의 best match count 와 비교해야 됨.
                                                best_match_count_prev = len(matches2)
                                                if len(matches) >= best_match_count_prev:
                                                    best_match = t
                                                    best_match_count = len(matches)
